...
FeedsAssistant.prototype.setup = function() {
    ...
    // set up sources
    this.feedSources = {
        webFeed: 'http://re-publica.de/11/feed/',
        ...
        snTag: 'http://identi.ca/api/statusnet/tags/timeline/rp11.atom', //
        twAccount: 'http://twitter.com/statuses/user_timeline/9824472.atom',
        twTag: 'http://search.twitter.com/search.atom?q=%23rp11'
    };
...
FeedsAssistant.prototype.processIncomingFeedItems = function( transport ) {
    ...
    if( that.listModel.items.length > 0 ) {
        ...
        if( that.activeFeed == 'webFeed' ) {
            that.listModel.listTitle = $L('re:publica website');
        }
        if( that.activeFeed == 'snAccount' ) {
